...
#(Normal HSCs Vs Remission ClassA at 6 months TKI).
###############################################################################
...
groupA.cells<-subset(groupA.cells,Stage_1=="6_months_TKI")
...
my.phenotype.s1<-rep("6_MONTHS_TKI",ncol(groupA.m))
...
h2<-paste("#","6_MONTHS_TKI","normalHSC",sep=" ")
h3<-paste(c(rep("6_MONTHS_TKI",length(groupA)),rep("normalHSC",length(groupB)),sep=" "))

cat(h1,file=paste("6_MONTHS_Vs_NormalHSC-GSEA-phenotype",".cls",sep=""),sep="\n")
cat(h2,file=paste("6_MONTHS_Vs_NormalHSC-GSEA-phenotype",".cls",sep=""),sep="\n",append=TRUE)
cat(h3,file=paste("6_MONTHS_Vs_NormalHSC-GSEA-phenotype",".cls",sep=""),append=TRUE)

write.table(my.final,file="6_MONTHS_Vs_NormalHSC-GSEA-format.txt",
